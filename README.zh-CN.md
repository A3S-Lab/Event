...
|事件版本控制/架构 | **A3S 活动** |与提供商无关的应用程序级别的关注点。 |
|有效负载加密 | **A3S 活动** |发布前应用程序级加密/解密。 |
|死信队列 | **A3S 活动** |跨提供商的统一 DLQ 抽象。 |
|状态持久化| **A3S 活动** |订阅过滤器在重新启动后的持久性。 |
|可观察性| **A3S 活动** |应用程序级指标和跟踪。 |
|提供者配置直通 | **A3S 活动** |公开提供者本机旋钮（`MaxDeliver`、`BackOff` 等）|
